...
class ServerSettings:
    ...
    def save_settings(self, name="default", force=False):
        '''Save the settings to the system. `name` can be used to create unique setting saves. If a custom name is
        provided and the name already exists, `force` will overwrite the file. '''
        ...
        if os.path.isfile(filename) and not force and not name == "default":
            raise ServerSettingsException(
                "Server setting file \"{}\" already exists. Use force=True to save to this location.".format(name))
